...
class CFM(torch.nn.Module):
    ...
    def inference(self, mu, x_lens, prompt, n_timesteps, temperature=1.0, inference_cfg_rate=0):
        ...
        for j in range(n_timesteps):
            ...
            v_pred = self.estimator(x, prompt_x, x_lens, t_tensor,d_tensor, mu,drop_audio_cond=False,drop_text=False, use_grad_ckpt=False).transpose(2, 1)
            if inference_cfg_rate>1e-5:
                neg = self.estimator(x, prompt_x, x_lens, t_tensor, d_tensor, mu, drop_audio_cond=True, drop_text=True, use_grad_ckpt=False).transpose(2, 1)
